...
function _trkHeatmapGrid(host, yms, byYm, allocs, efLoans, thisYm, mod, now) {
  ...
  row('Household budget', 'trk-heat-kitty', cols.map((k) => ({ text: money(kittyOf(k)) })));
...
const _mfCell = (k, v, cls, help) => el('div', { class: 'cell' }, [
...
function importSheetCSV() {
  ...
  input.addEventListener('change', async () => {
    ...
    try {
      ...
      appAlert('Could not read CSV: ' + e.message);
      return;
    }
    const stocks = parsed.stocks || [];
    const monthly = parsed.monthly || [];
    if (!stocks.length) {
      appAlert('No stock rows found. Export the "Stock" tab from your sheet as CSV and try again.');
      return;
    }
    const by = {};
