...
- Dates use whole-day math (`365.25`-day years, UTC parse) so there's no timezone drift; `addMonths` fills the maturity date from a typed tenure. (An inclusive-day/flat-365 convention was tried and reverted by the user — it matched one real FD to the paisa but the exclusive/365.25 rule was preferred overall. Different banks use different day-count conventions; no single formula matches every bank to the rupee.)
