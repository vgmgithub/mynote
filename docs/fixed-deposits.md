...
- **FDs (holdings)** — filter `Active | Matured | All` (live counts) + sort `Maturity (soonest) | Amount | Rate` + the FD card list. Each card: bank, `rate% · compounding` + status badge + a compact blue **`reinvested`** badge (`.badge.fd-reinvested`) when the FD was funded by rolling in matured FD(s), + a grey `rolled over` badge if it's been reinvested onward; interest headline, invested (effective, rounded to whole rupees), maturity date + days-to-maturity, and the maturity-value card (also whole-rupee). The fresh/rolled breakdown lives in the Chain tab, not on the card (keeps it to one line). **Superseded matured FDs are hidden** from the list — a matured FD whose child has *also* matured is absorbed into that newer link, so the matured list shows only the latest matured link per chain and can't grow unbounded. (Superseded FDs stay in the data and are visible via the Chain tab.)
