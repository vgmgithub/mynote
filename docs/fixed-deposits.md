...
- **Overview** — summary card modelled on the **rolling ladder** (reinvest matured proceeds into new FDs): headline *Total invested value* (= **active**-FD principal, as-is) with a *Current invested* sub-line (= Total invested − Reinvested (P+I) — the fresh capital still your own once recycled matured proceeds are stripped back out), *Interest to earn* (active), then a grid of *Reinvested (P+I)* (Σ maturity value of matured FDs — the proceeds rolled into new FDs) / *Interest matured* (green — Reinvested minus matured principal, i.e. interest already realized from matured FDs; forward-looking "Interest to earn" above is for active FDs, this is the realized counterpart for matured ones) / *Avg rate* / **Portfolio XIRR** / *Active FDs*. **Avg rate** is a plain principal-weighted average of each active FD's nominal annual rate (`Σ(rate×principal)/Σprincipal`) — blind to tenure, since a % rate doesn't need it. **Portfolio XIRR** is the tenure-aware counterpart: reuses `mf.js`'s generic `xirr(cashflows)` (not MF-specific — plain IRR over dated cashflows), treating each active FD's principal as a cashflow out on its start date and its maturity value as a cashflow in on its maturity date, then solving for the single annualized rate that reconciles the whole ladder — so a longer-tenure FD's compounding correctly pulls the blended figure, which a principal-only weighting can't see. Verified example: FDs of 1yr/3yr/1yr tenure at 7.0%/8.0%/7.5% → Avg rate 7.39%, Portfolio XIRR 7.86% (quarterly compounding pushes the effective annualized return above the nominal blend). Payout FDs' periodic payouts aren't modelled as separate cashflows (same lump-at-maturity simplification `computeFd` already uses for their `totalInterest`). Shows `—` if fewer than 2 active FDs have both dates set. **Broken FDs are excluded** from these ladder totals (early exit, not a rung). Below: **Invested by bank** (`.bar-row`) + **Interest income potential** (avg ₹/month + ₹/year across active FDs) + **Next maturity**.
