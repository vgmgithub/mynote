...
Every code change bumps `CACHE = 'mynote-stocks-vNN'` in `service-worker.js`. Current version after the Home FD-card invested tweak (total invested principal, active + matured; see [fixed-deposits.md](fixed-deposits.md)): **v148**. The next change should be v149.
