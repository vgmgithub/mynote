...
8. **[gotchas.md](gotchas.md)** — bugs that cost real time. Read before debugging "the app isn't updating" — it's almost always cache.
9. **[future.md](future.md)** — discussed but not built. Don't pick these up unprompted; user has views on each.

## Project at a glance

- **What:** A private, offline-first PWA for tracking 3 portfolios (Me·India, Wife·India, Me·US) — monthly returns, heatmap, insights, OCR-from-broker-screenshot price updates.
...
Every code change bumps `CACHE = 'mynote-stocks-vNN'` in `service-worker.js`. Current version after the latest docs/features update: **v64**. The next change should be v65.
