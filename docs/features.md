...
- **Per-stock card**: shows current value, overall return %, invested, units, latest monthly change. Sold cards instead show the exit verdict badge and three comparison figures, each vs. the same buy price so they read together: **Booked** (realised P/L, `soldUnits × (soldPrice − buyPrice)`), **If held** (hypothetical P/L had it not been sold, `soldUnits × (currentPrice − buyPrice)`), and **vs. exit** (the difference between the two — coloured by the exit verdict, not by the number's own sign, since a price rise after a *good* exit is money missed and prints red even though the figure is "+"). When a sold stock has no recorded buy price (e.g. an old CSV import), neither P/L figure exists, so **Booked** falls back to **Sold for** (gross proceeds only) and **If held** falls back to the raw value at today's price — **vs. exit** still works either way, since it only needs the sale price and today's price.
