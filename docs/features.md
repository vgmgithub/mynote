...
- **Per-stock card**: shows current value, overall return %, invested, units, latest monthly change. Sold cards instead show the exit verdict badge and three raw-value comparison figures — none need a buy price, so all three work even for a CSV import with no cost basis on record: **If held** (`soldUnits × currentPrice`), **Booked** (`soldUnits × soldPrice`), and **vs. exit** (If held minus Booked — positive means you'd have been better off holding, shown green; negative means you did the right thing selling, shown red). Each figure renders independently once its own inputs exist (e.g. Booked alone if the current price isn't set yet).
