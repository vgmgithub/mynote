...
- **Per-stock card**: shows current value, overall return %, invested, units, latest monthly change. Sold cards instead show the exit verdict badge, a signed **"vs. exit"** line comparing today's price to the sale (coloured by the exit verdict, not by the number's own sign — a price rise after a *good* exit is money missed, so it prints red even though the figure is "+"), and two comparison figures: **Booked** (realised P/L, `soldUnits × (soldPrice − buyPrice)`) and **If held** (`soldUnits × currentPrice`) — so a sold position can be judged against "what if I'd kept it?". When a sold stock has no recorded buy price (e.g. an old CSV import), **Booked** falls back to **Sold for** (gross proceeds only, no P/L claimed since the cost basis is unknown) while **If held** still compares against today's price.
