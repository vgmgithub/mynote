...
- **Per-stock card**: shows current value, overall return %, invested, units, latest monthly change. Sold cards instead show the exit verdict badge and, when a buy price is on record, three comparison figures vs. that same buy price: **Booked** (realised P/L, `soldUnits × (soldPrice − buyPrice)`), **If held** (hypothetical P/L had it not been sold, `soldUnits × (currentPrice − buyPrice)`), and **vs. exit** (the difference between the two, coloured by whether the sale itself was profitable — a booked loss stays red even if it avoided a bigger one, a booked profit stays green even if more was left on the table). When a sold stock has no recorded buy price (e.g. an old CSV import), there's no cost basis to measure any of the three against, so none of them render — only the badge and the sale/current-price lines show.
