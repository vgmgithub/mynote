...
```js
{
  ...
  interestFreq,            // optional — when coupons arrive: 'monthly'|'quarterly'|'halfyearly'|'yearly'|'maturity'|'staggered'. Payout bonds only (a cumulative bond IS "at maturity", so the form hides the picker and stores null). null/absent = unspecified → no schedule, totals behave exactly as before this field existed
  principalFreq,           // optional — when principal comes back: same values. Absent/'maturity' = the classic single lump. Anything else means the bond AMORTIZES
